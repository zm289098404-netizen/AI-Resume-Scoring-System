...
REM ---- 1. 创建 / 选择项目专属 Python ----
if not exist "%PROJECT_PY%" (
    echo  [+] 未检测到项目专属虚拟环境，正在创建 .venv ...
...
echo  [+] 当前使用解释器: %PROJECT_PY%

REM ---- 2. 跑 bootstrap (环境检查 + 依赖安装 + 向导) ----
"%PROJECT_PY%" bootstrap.py
if errorlevel 1 (
    echo.
...
"%PROJECT_PY%" -m streamlit run src\talentscope\ui\app.py --server.headless false --server.port 8501 --browser.gatherUsageStats false
