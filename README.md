...
- **🧩 岗位补位后备池**：自动分层 A/B/C 候选池，输出岗位连续性风险与补位建议
- **📐 严谨模板库**：岗位模板含职责、硬性要求、业务理由、核验证据与风险控制点
- **🚨 人才流失预警**：关键部门即战力人数低于阈值自动告警
...
1. 在 JD 匹配评分页输入岗位需求并设置补位目标人数
2. 优先从人才库选人，系统输出评分证据与岗位后备池
3. 依据 A/B/C 后备层级安排面试与培养动作
4. 面试后将结果写入反馈闭环，沉淀到评测指标
5. 每周复盘反馈覆盖率和正向反馈率，持续校准模板与门槛

---

## 📐 模板库治理

- 模板定义位于 `config/job_templates.json`
- 字段规范与质量门槛见 `docs/JOB_TEMPLATE_SPEC.md`
- 每个硬性要求需包含：要求、业务理由、核验证据提示
- 模板升级建议按版本号管理并做回归验证
