...
def match(jd: dict, resume: dict) -> dict:
    ...
    # ✅ 改进1：使用安全的JSON解析
    llm_response = client.chat_json(SYSTEM_PROMPT, payload)
    llm_result = safe_json_loads(
        json.dumps(llm_response) if isinstance(llm_response, dict) else str(llm_response),
        context=f"jd={jd.get('title', 'unknown')}"
    )
    
    # ✅ 改进2：检查是否为错误对象
    if "_error" in llm_result:
        logger.warning(f"LLM返回异常: {llm_result.get('_error')}")
        llm_result = _build_fallback_result(jd, resume, llm_result.get("_error"))
    ...
    # ✅ 改进3：添加验证上下文
    llm_result["_jd_hard_skills_count"] = len(
        [s for s in jd.get("hard_skills", []) if s.get("required", True)]
    )
    
    # ✅ 改进4：质量验证
    is_valid, error_msg, fixed_result = MatchResultValidator.validate(llm_result)
    
    if not is_valid:
        logger.warning(f"匹配结果验证失败: {error_msg}, 已自动修复")
        fixed_result["_quality_issues"] = [error_msg]

    # 兜底字段
    fixed_result.setdefault("recommendation", llm_result.get("recommendation", "—"))
    fixed_result.setdefault("risks", [])
    fixed_result.setdefault(
        "interview_questions",
        _build_interview_questions(jd, resume, local),
    )
    
    # ✅ 改进5：计算置信度
    fixed_result["confidence"] = _calculate_confidence(
        is_valid,
        local.get("confidence", 0.7),
...
def _build_fallback_result(jd: dict, resume: dict, error: str) -> dict:
    """LLM故障时的完全降级方案"""
    local = _local_score(jd, resume)
    return {
        **local,
...
def _calculate_confidence(
    ...
    """计算置信度 (0-1)"""
    if not is_valid:
        return local_confidence * 0.7
    
    jd_required = {s["name"] for s in jd_hard_skills if s.get("required", True)}
    if not jd_required:
        return local_confidence
    
    matched_rate = len(set(matched_skills) & jd_required) / len(jd_required)
    coverage_confidence = 0.5 + matched_rate * 0.5
    
    return min(1.0, (local_confidence + coverage_confidence) / 2)
